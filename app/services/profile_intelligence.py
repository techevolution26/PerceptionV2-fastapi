...
def build_profile_intelligence(
    *,
    ...
    semantic_rows: list[
        tuple[CommentIntelligence, int, int | None, str | None, datetime]
    ],
    ...
    by_topic: dict[tuple[int | None, str], list[CommentIntelligence]] = defaultdict(
        list
    )
    ...
    perception_topic = {
        p.id: (p.topic_id, p.topic.name if p.topic else "Uncategorized")
        for p in perceptions
    }
    qualifying_perceptions = sum(
        len(by_perception.get(p.id, [])) >= minimum for p in perceptions
    )
    ...
    for (topic_id, topic_name), topic_rows in sorted(
        ...
        topic_sections.append(
            {
                ...
                "sentiment_distribution": _distribution(
                    [r.sentiment for r in topic_rows]
                ),
    ...
    recurring_themes = [
        ...
        for theme, topic_set in sorted(
            theme_topics.items(),
            key=lambda item: (len(item[1]), theme_comments[item[0]]),
            reverse=True,
        )
    ...
    while bucket_end > period_start:
        bucket_start = max(
            period_start, bucket_end - timedelta(days=PROFILE_BUCKET_DAYS)
        )
        bucket_rows = [
            intelligence
            ...
            buckets.append(
                {
                    ...
                    "status": "available",
                    "sentiment_distribution": _distribution(
                        [r.sentiment for r in bucket_rows]
                    ),
                    "stance_distribution": _distribution(
                        [r.stance for r in bucket_rows]
                    ),
                    "top_themes": themes,
        ...
        patterns.append(
            {
                ...
                "limitations": [
                    "Theme recurrence is observational and does not establish why the theme recurs."
                ],
            }
        )
        ...
        patterns.append(
            {
                ...
                "limitations": [
                    "Topic-level samples are not necessarily population-representative."
                ],
            }
        )
    if len(buckets) >= 2:
        first_stance = next(
            (x["label"] for x in buckets[0]["stance_distribution"]), None
        )
        last_stance = next(
            (x["label"] for x in buckets[-1]["stance_distribution"]), None
        )
            ...
            patterns.append(
                {
                    ...
                    "sample_size": min(
                        buckets[0]["sample_size"], buckets[-1]["sample_size"]
                    ),
                    "limitations": [
                        "A change in observed stance does not establish causation or a durable change in audience attitude."
                    ],
                }
            )
    ...
    return {
        ...
        "topic_count": len(
            {
                perception_topic[p.id][0]
                for p in perceptions
                if perception_topic[p.id][0] is not None
            }
        ),
