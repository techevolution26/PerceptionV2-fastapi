...
async def assess_topic_freshness(
    db: AsyncSession,
    perception_ids: list[int],
    since: datetime,
) -> dict:
    """Topic-scoped variant of :func:`assess_intelligence_freshness`.

    Same status semantics (current / pending / stale), aggregated across every
    Perception in the requested Topic scope instead of a single Perception.
    Reuses the exact same rules rather than introducing a second freshness
    engine; only the query scope (many Perceptions, not one) differs.
    """
    if not perception_ids:
        return {
            ...
            "latest_source_at": None,
            "latest_analysis_at": None,
            "note": "No Perceptions are present in the selected Topic scope.",
        }

    comments = list((await db.scalars(
        select(Comment)
        .where(Comment.perception_id.in_(perception_ids), Comment.created_at >= since)
    )).all())
    comment_ids = {comment.id for comment in comments}
    ...
    if not comment_ids:
        return {
            ...
            "latest_source_at": None,
            "latest_analysis_at": None,
            "note": "No comments are present in the selected Topic period.",
        }
    ...
    if missing or pending:
        status = "pending"
        note = "New or queued comments under this Topic are awaiting semantic analysis."
    elif failed or needs_recalculation:
        status = "stale"
        note = "Stored Topic intelligence is older than the current conversation and requires recalculation."
    else:
        status = "current"
        note = "Stored Topic intelligence covers the current comments in the selected period."
