...
- sentiment is the comment's expressed emotional orientation toward the discussion.
    ...
    try:
        async with httpx.AsyncClient(
            ...
            if response.status_code == 429:
                raise CommentIntelligenceProviderError(
                    "provider_rate_limited",
                    retry_after_seconds=_retry_after_seconds(response),
                )
            if response.status_code in {401, 403}:
                raise CommentIntelligenceProviderError("provider_auth_error")
            if 500 <= response.status_code <= 599:
                raise CommentIntelligenceProviderError("provider_server_error")
            if 400 <= response.status_code <= 499:
                raise CommentIntelligenceProviderError("provider_http_error")
            response.raise_for_status()
            data = response.json()
    except CommentIntelligenceProviderError:
        raise
