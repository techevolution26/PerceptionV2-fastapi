...
class User(TimestampMixin, Base):
    ...
    verification_status: Mapped[str] = mapped_column(
        String(32), default="NOT_APPLIED", nullable=False
    )
...
class Perception(TimestampMixin, Base):
    ...
    likes: Mapped[list["Like"]] = relationship(
        back_populates="perception", cascade="all, delete-orphan"
    )
...
class Like(TimestampMixin, Base):
    __tablename__ = "likes"
    __table_args__ = (
        UniqueConstraint("user_id", "perception_id", name="uq_like_user_perception"),
    )
...
class Message(Base):
    ...
    __table_args__ = (
        Index("ix_messages_pair", "from_user_id", "to_user_id", "created_at"),
    )
...
class VerificationApplication(Base):
    ...
    __table_args__ = (Index("ix_verification_user_status", "user_id", "status"),)
    ...
    requested_topic_ids: Mapped[list] = mapped_column(
        JSON, default=list, nullable=False
    )
...
class PerceptionModeration(Base):
    ...
    __table_args__ = (
        Index("ix_perception_moderation_status_created", "status", "checked_at"),
    )
