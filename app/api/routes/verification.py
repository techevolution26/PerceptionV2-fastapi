...
@router.post(
    "/applications",
    response_model=VerificationApplicationOut,
...
async def apply(
    ...
    if not sub.plan.verification_included:
        raise HTTPException(
            status_code=403,
            detail="Your plan does not include professional verification.",
        )

    if current_user.verification_status in {"PENDING", "VERIFIED"}:
        raise HTTPException(
            status_code=409,
            detail="You already have an active verification application.",
        )
    ...
    try:
        validate_identity_selection(
            industry_codes, role_codes, payload.primary_professional_role
        )
    ...
    if not role_codes:
        raise HTTPException(
            status_code=422,
            detail="Select at least one professional role before applying for verification.",
        )
    ...
    current_user.analytics_specialties = topic.ids
...
async def admin_list_applications(admin: SuperAdminUser, db: DbSession):
    result = await db.execute(
        select(VerificationApplication).order_by(
...
async def admin_review_application(
    ...
    if application is None:
        raise HTTPException(
            status_code=404, detail="Verification application not found"
        )
    ...
    user.verified_professional_roles = (
        list(application.professional_role_codes) if approved else []
    )
    db.add(
        AdminAuditLog(
            ...
            action=(
                "professional_verification.approved"
                if approved
                else "professional_verification.rejected"
            ),
            data={
                "application_id": application.id,
                "roles": application.professional_role_codes,
                "industries": application.industry_codes,
            },
        )
    )
    if approved:
